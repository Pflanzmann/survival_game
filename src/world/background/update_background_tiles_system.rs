...
pub fn update_background_tiles_system(
    ...
    let mut tile_matrix: Vec<Vec<bool>> = vec![vec![false; 30]; 30];
    let current_grid_position = (background_tiles.current_origin / 256.0).round();

    for (entity, transform) in tile_query.iter_mut() {
        let grid_grid_position = (transform.translation.truncate() / 256.0).round();
        let local_position = grid_grid_position - current_grid_position;

        if local_position.x > 29.0 || local_position.y > 29.0 || local_position.x < 0.0 || local_position.y < 0.0 {
    ...
    for (x_index, x) in tile_matrix.iter().enumerate() {
        for (y_index, y) in x.iter().enumerate() {
            ...
            let local_x = (x_index as i32 * 256) as f32 + background_tiles.current_origin.x;
            let local_y = (y_index as i32 * 256) as f32 + background_tiles.current_origin.y;

            let noise_x = (x_index as f64 + current_grid_position.x as f64) / 30.0;
            let noise_y = (y_index as f64 + current_grid_position.y as f64) / 30.0;
