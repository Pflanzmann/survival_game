...
pub fn solid_body_collision_system(
    ...
    for (entity, mut transform, collider_type, collision_weight) in solid_body_unit_query.iter_mut() {
        ...
        for quad_data in check_entity_list.iter() {
            ...
            if collider_type.is_colliding(&transform.translation.truncate(), &quad_data.data.collider_type, &quad_data.position.truncate()) {
                let resolution_position = collider_type.get_collision_resolution_position(&transform.translation.truncate(), collision_weight.weight, &quad_data.data.collider_type, &quad_data.position.truncate(), quad_data.data.collision_weight);
