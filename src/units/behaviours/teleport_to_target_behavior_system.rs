...
pub fn teleport_to_target_behavior_system(
    mut unit_query: Query<(&TurretOwner, &mut Transform), (With<TeleportToTargetBehavior>, Without<Player>)>,
    player_query: Query<(Entity, &Transform), With<Player>>,
) {
    for (player_entity, player_transform) in player_query.iter() {
        for (owner, mut turret_transform) in unit_query.iter_mut() {
            if player_entity == owner.owner && player_transform.translation.distance(turret_transform.translation) > 2000.0 {
