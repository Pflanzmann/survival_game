...
pub fn steering_behavior_system(
    ...
    units_query.par_for_each_mut( 10, |(entity, transform, solid_body_collider, move_direction, mut steering_behavior)| {
        ...
        for quad_data in check_entity_list.iter() {
            ...
            if solid_body_collider.collider_type.is_colliding(&check_position, &quad_data.data.collider_type, &quad_data.position) {
                let distance = unit_position.distance(quad_data.position);
